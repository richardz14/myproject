...
'  </code>
'That will prevent the TabHost from stealing the focus (a known bug, still not fixed in Android 4.0.3).
Public Sub ExplorerMulti As typResult
...
'  </code>
'That will prevent the TabHost from stealing the focus (a known bug, still not fixed in Android 4.0.3).
Public Sub ExplorerMulti2(DarkTheme As Boolean) As typResult
	bMultiFolderSelection = bOnlyFolders
	bMultiFileSelection = Not(bOnlyFolders)
	Explorer2(DarkTheme)
End Sub
...
Private Sub CommonExplorer
	...
	Try
	'	svFichiers.GlowEffect(svFichiers.GLOW_EFFECT_ALL_SIDES, True)
	'	svFichiers.GlowEffect(svFichiers.GLOW_EFFECT_LEFT, False)
	'	svFichiers.GlowEffect(svFichiers.GLOW_EFFECT_RIGHT, False)
	Catch
		'The GlowEffect function is not available with SV2D v1.3
	End Try
